...
export default function Navbar({ stickyTop = 0 }) {
  ...
  const linkClass = ({ isActive }) =>
    ...
    } after:content-[''] after:absolute after:left-0 after:bottom-0 after:h-px after:bg-brass after:transition-all ${
  ...
  return (
    <header
      ...
      className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between transition-all duration-300 ${
        scrolled
          ? 'py-3 px-6 md:px-7 bg-hangardeep/90 backdrop-blur-md border-b border-ink/10'
          : 'py-5 px-6 md:px-7'
      }`}
    >
      <Link to="/" className="flex items-center gap-2 font-display font-bold text-xl tracking-wide">
        <svg viewBox="0 0 48 48" fill="none" className="w-7 h-7">
          <path
            d="M24 4 L28 20 L44 26 L28 28 L24 44 L20 28 L4 26 L20 20 Z"
            stroke="currentColor"
            strokeWidth="2"
          />
        </svg>
      ...
      <nav className="hidden md:flex items-center gap-8 font-mono text-[0.78rem] tracking-wider uppercase">
