...
// Compact landing-pad reticle — recolored to brass so it reads as part of
// the club's palette instead of a stark white HUD crosshair.
function TacticalLandingPad() {
  return (
    ...
    <>
      ...
      <section className="py-16 md:py-24 overflow-x-hidden">
        <div className="max-w-[800px] mx-auto px-5 sm:px-6">
          ...
          {/* Squad filter — same pill style as the Team page for consistency */}
          ...
          <div className="relative space-y-12">
            <div className="absolute top-0 bottom-0 left-6 sm:left-8 w-px -translate-x-1/2 bg-ink/10 z-0" />

            {TIMELINE_DATA.map((group) => {
              const filteredEntries = group.entries.filter((item) =>
                activeFilter === 'ALL' ? true : item.team.toUpperCase() === activeFilter
              )
              if (filteredEntries.length === 0) return null

              const isOpen = openYear === group.year
              const victoryCount = filteredEntries.filter((e) => e.isMajorVictory).length
